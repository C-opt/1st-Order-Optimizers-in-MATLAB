...
parfor idx = 1: number_of_experiments
    fprintf('------------------------------EXPERIMENT NO. %d------------------------------\n', idx);              
                                                              %DASVRDA_adapRestart_pflug_ns(X_train, Y_train, x_tilde, z_tilde, omega, L, m, b, S, eta, lambda1, lambda2, experiment_boolean, innerPt_no)
    [all_S_pflug_over(:,idx), all_eta_over(:,idx), ~, ~, ~] = DASVRDA_adapRestart_pflug_ns(X_train, Y_train, zeros(data_dim, 1), zeros(data_dim, 1), omega, L, m, b, S_adres, eta2, lambda1, lambda2, 0, innerPt_no); 
    [all_S_pflug_under(:,idx), all_eta_under(:,idx), ~, ~, ~] = DASVRDA_adapRestart_pflug_ns(X_train, Y_train, zeros(data_dim, 1), zeros(data_dim, 1), omega, L, m, b, S_adres, eta3, lambda1, lambda2, 0, innerPt_no); 
end
